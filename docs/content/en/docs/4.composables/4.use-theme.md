...
::note
The font is not managed by `useTheme`: it is build-time configuration, declared as `--font-sans` in the `@theme` block of `app/assets/css/main.css`. Chinese fonts are split by `unicode-range`, so switching at runtime would mean re-downloading several MB of shards — hence no switching is offered.
::
