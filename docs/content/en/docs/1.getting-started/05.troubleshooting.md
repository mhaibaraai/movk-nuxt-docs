...
::warning
Chinese fonts are split by `unicode-range`, and a single family easily runs to hundreds of shards. Do not hand them to `@nuxt/fonts` — it downloads every shard into the build output at build time, losing both on-demand loading and the CDN cache shared across projects.
::
...
Starting from pnpm v10, dependency lifecycle scripts (such as `postinstall`) are not run by default; since v11 approvals are declared through the `allowBuilds` map (the older `onlyBuiltDependencies` and `ignoredBuiltDependencies` settings have been removed). This project requires approval for the following dependencies that include native modules:
...
::warning{to="https://pnpm.io/settings/build#allowbuilds"}
