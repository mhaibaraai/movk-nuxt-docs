...
```
  ...
  ::tip
  AI Chat is enabled as long as `AI_API_KEY` is present; if it is missing, the module is disabled and logs a message to the console.
  ::
...
| Alibaba Cloud Qwen (international) | `https://dashscope-intl.aliyuncs.com/compatible-mode/v1` |
| Alibaba Cloud Qwen (China) | `https://dashscope.aliyuncs.com/compatible-mode/v1` |
| Zhipu GLM | `https://open.bigmodel.cn/api/paas/v4` |

::note
One `AI_API_KEY` + `AI_BASE_URL` pair maps to a single upstream, so keep `models` to that provider's models. To reach several providers at once, register custom providers as shown above.
::
