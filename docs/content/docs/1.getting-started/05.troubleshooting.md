...
::warning
中文字体经 `unicode-range` 分包，一个家族动辄数百个分片。不要把它们交给 `@nuxt/fonts`——它会在构建期把全部分片下载进产物，既失去按需加载，也丢掉跨项目共享的 CDN 缓存。
::
...
从 pnpm v10 开始，默认情况下不会运行依赖的 lifecycle scripts（如 `postinstall`）；v11 起审批结果统一通过 `allowBuilds` 映射声明（`onlyBuiltDependencies`、`ignoredBuiltDependencies` 等旧设置已移除）。本项目需要审批以下包含原生模块的依赖：
...
::warning{to="https://pnpm.io/zh/settings/build#allowbuilds"}
了解更多关于构建脚本审批的信息
::
