---
title: 项目结构
description: 深入了解 Movk Nuxt Docs 项目的目录结构与各文件职责，包括 content/ 内容目录、public/ 静态资源、nuxt.config.ts 模块配置和 app.config.ts 运行时 UI 配置的详细说明及典型配置示例。
seo:
...
系统会自动检测 `releases.yml` / `releases.md` 与 `templates.yml` / `templates.md` 文件，并分别创建 `/releases` 与 `/templates` 路由，无需手动配置。
::
