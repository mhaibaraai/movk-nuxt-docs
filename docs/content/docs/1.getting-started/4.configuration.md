...
覆盖 `app.config.ts` 需要同时创建 `nuxt.config.ts` 文件。
::

### Vercel Analytics

在 `app.config.ts` 中启用 Vercel Analytics：
...
在 `app.config.ts` 中配置 AI Chat 的界面文本、图标、快捷键和 FAQ 问题。
