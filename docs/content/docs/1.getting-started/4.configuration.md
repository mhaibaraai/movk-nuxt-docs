...
对于 `CommitChangelog` 组件的高级配置，您可以设置以下属性：
...
export default defineAppConfig({
  github: {
    ...
    rootDir: 'docs',         // 文档根目录
    commitPath: 'src',       // 组件文件路径(默认: 'src')
    suffix: 'vue',           // 组件文件后缀(默认: 'vue')
    since: '2025-01-31T04:00:00Z',  // 提交历史起始时间(ISO 8601 格式)
  },
})
```
...
`commitPath`、`suffix` 和 `since` 主要用于 `CommitChangelog` 组件。了解更多请查看 CommitChangelog 文档。
::
