...
seo:
  description: Movk Nuxt Docs 完整配置参考：app.config.ts 控制 SEO、Header、Footer、TOC、AI Chat 界面，nuxt.config.ts 配置 GitHub 集成、componentMeta、Mermaid 和 a11y 模块。
...
| 选项 | 默认值 | 说明 |
|------|--------|------|
| `a11y` | `true`{lang="ts-type"} | 是否启用 `@nuxt/a11y` 无障碍支持。|
| `mermaid` | `false`{lang="ts-type"} | 是否启用 Mermaid 图表渲染（需安装 `mermaid` 与 `dompurify`）。|

::tip{to="/docs/typography/mermaid"}
...
Movk Nuxt Docs 默认集成 [@nuxt/a11y](https://a11y.nuxtjs.org/) 模块，提供：
