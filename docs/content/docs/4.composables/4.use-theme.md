---
title: useTheme
description: useTheme 是主题定制系统的核心 Composable，管理 Primary 主色调、Neutral 中性色、圆角大小、字体系列和图标集配置，支持深色/浅色/系统三种颜色模式。可从 ThemePicker 导出 CSS 变量或 app.config.ts 配置持久化主题。
seo:
  title: useTheme
