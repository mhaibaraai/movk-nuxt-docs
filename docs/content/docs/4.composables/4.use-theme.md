---
title: useTheme
description: useTheme 是主题定制系统的核心 Composable，管理 Primary 主色调、Neutral 中性色、圆角大小和图标集配置，支持深色/浅色/系统三种颜色模式。可从 ThemePicker 导出 CSS 变量或 app.config.ts 配置持久化主题。
seo:
  title: useTheme
  description: Customize Movk Nuxt Docs theming with useTheme, including primary and neutral colors, radius, icons, color modes, and exported CSS variables.
links:
  - label: GitHub
...
`useTheme` 提供完整的主题定制能力，允许用户动态调整站点的视觉风格，包括颜色方案、圆角大小、图标集以及明暗模式切换。所有配置自动持久化到 localStorage。
...
::note
字体不由 `useTheme` 管辖：它是构建期配置，在 `app/assets/css/main.css` 的 `@theme` 中声明 `--font-sans` 即可。中文字体经 `unicode-range` 分包，运行时切换意味着重新下载数 MB 分片，因此不提供切换能力。
::
...
::field-group
  ...
  中性色，可选值：`slate` / `gray` / `zinc` / `neutral` / `stone` / `taupe` / `mauve` / `mist` / `olive`。
  :::
  ...
  计算属性，判断是否有 CSS 相关的自定义配置（圆角、字体、黑色主色、自定义颜色、CSS 变量）。
  :::
