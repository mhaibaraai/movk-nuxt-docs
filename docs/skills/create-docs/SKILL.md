...
| `references/writing-guide.md` | 内容结构、写作规范和页面模板 | 创建新页面或组织内容时 |
| `references/skill-structure.md` | Skill 目录结构与 front-matter 规范 | 新增或维护 skills 时 |

## 编写 Agent Skills（高级）

如果您希望为仓库新增 skill，请遵循以下约定：
...
- `name` 必须与目录名一致
- 使用 kebab-case（仅 `a-z`、`0-9`、`-`）
- 不能包含连续 `--`
- 最大长度 64 字符

### 发布与验证

- 站点会自动扫描 `skills/*/SKILL.md` 并发布到 `/.well-known/skills/*`
- 建议新增或修改后验证：
  - `/.well-known/skills/index.json`
  - `/.well-known/skills/{skill-name}/SKILL.md`

完整规范见 `references/skill-structure.md`。

## 相关技能

文档编写完成后，使用 `review-docs` 技能进行质量审查：
- 验证 Front-Matter 完整性
- 检查 MDC 语法正确性
- 评估内容清晰度和 SEO
- 检查结构和导航合理性
...
- [ ] 代码块标注了语言标识符和文件名（如 ` ```ts [nuxt.config.ts] `）
- [ ] 多包管理器安装示例使用 `::tabs`（而非 `::code-group`）
- [ ] 中英文之间有空格
- [ ] 使用中文全角标点
- [ ] 图片放在 `public/` 目录
- [ ] 指南页面包含"下一步"链接（`::card-group` 或 Front-Matter `links`）
