...
### AI 助手 Skill（`layer/modules/skills/`）

`skills` 模块在构建时扫描消费项目的 `skills/` 目录，将所有符合规范的 Skill 自动发布为两个 HTTP 端点：

| 端点 | 说明 |
|------|------|
| `GET /.well-known/skills/index.json` | 所有 Skills 的清单（name、description、files） |
| `GET /.well-known/skills/:name/:path` | 指定 Skill 的文件内容（SKILL.md、references/\*） |

**在 `docs/skills/` 下添加新 Skill：**

```
skills/
└── my-skill/             # 目录名须与 name 字段完全一致
    ├── SKILL.md          # 必需：frontmatter + 工作流说明
    └── references/       # 可选：详细参考文档
```

`SKILL.md` 必需的 frontmatter 字段：
...
命名规则：仅小写字母、数字、连字符；不以连字符开头或结尾；不含连续连字符（`--`）。不符合规范的 Skill 在构建时会被跳过并输出警告。
